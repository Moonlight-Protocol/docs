...
Because the provider runs entirely off-ledger between wallet and channel, it becomes the natural point for jurisdiction-specific checks. A wallet acting as a privacy provider can authenticate the holder through a SEP-10 web session tied to the master secret, perform its own KYC, and keep an encrypted audit log—all without writing customer data to the blockchain. The same flow lets providers append a service fee or subsidise costs, creating flexible business models around privacy as a service.
...
### Programmable Channels: Customizable On-chain Privacy
